...
void Game::Update()
{
	...
	glm::vec3 carCenter = carPos + carShift + rightShift;
	glm::vec3 position = carPos + carShift;
	float size = 39;
	float road = 7;
	float border = 2;
	if (carCenter.x > border && carCenter.z > border && carCenter.x < (size - border) && carCenter.z < (size - border)) {
		if (carCenter.x < road || carCenter.z<road || carCenter.x >(size - road) || carCenter.z >(size - road))
			carPos = position;
	}
